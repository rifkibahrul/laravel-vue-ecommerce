    ...
    <div class="bg-white p-4 rounded-md shadow animate-fade-in-down">
        <div class="flex justify-between border-b-2 pb-3">
            <div class="flex items-center">
                ...
                <span class="ml-3">Found {{ users.total }}</span>
            </div>
            <!-- Search Bar Start -->
            <div>
                <input
        ...
        <table class="table-auto w-full">
            ...
            <tbody v-else>
                <tr v-for="user of users.data" v-bind:key="user.id">
        ...
        >
            <div v-if="users.data.length">
                Showing from {{ users.form }} to {{ users.to }}
            </div>
            <nav
...
function deleteUser(user) {
    ...
    store.dispatch('deleteUser', user.id)
        .then(res => {
            ...
            store.commit('showToast', `User was successfully deleted`)
            store.dispatch("getUsers");
        });
}
